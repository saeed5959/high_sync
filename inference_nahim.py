...
from image_processor_nahim import ImageProcessor
...
def inference_process(args: argparse.Namespace):
    ...
    with open(args.bbox, "r") as file:
        bbox_dict = file.readlines()	

    bbox_multi_speaker = []
    for bbox in bbox_dict:
        bbox_multi_speaker.append(ast.literal_eval(bbox))

    bbox_multi_speaker = bbox_multi_speaker[:video_length]
    ...
    while 1:
        ...
        if ((count_motion % config.inference_frames_num) == 0) or finish:
            ...
            source_image_pixels_all, bbox_all, frame_list_all = image_processor.preprocess_frames_jump(frames_video, bbox_multi_speaker[previous:previous+len(frames_video)])   
            previous += len(frames_video)         
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source_video", type=str, required=True, help="source image")
    parser.add_argument("--bbox", type=str, required=True, help="bbox")
    parser.add_argument("--driving_audio", type=str, required=True, help="driving audio")
